...
class NAEKF(GPS_IM_Classifier):
    ...
    def im(self, PL, epsilon, p, sigma, simEnv, offset):
        ...
        for i in range(Ndt):
            ...
            low = -1 * PL
            high = PL
            prob = multivariate_normal.cdf(high, mean=mu_enu.flatten(), cov=sigma_enu) - multivariate_normal.cdf(low, mean=mu_enu.flatten(), cov=sigma_enu)

            imvn = None  # You can compute log-probs or pdf if required as a substitute
